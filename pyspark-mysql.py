...
#import findspark
#findspark.init('/home/debatosh/spark/spark-3.0.0-bin-hadoop3.2')
...
dsraw = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "192.168.245.160:9092").option("subscribe", "test-output").option("startingOffsets", "earliest").load()
...
Person_details_df2 = ds.select(from_json(col("value"), custom_schema).alias("Person_details"))
...
def foreach_batch_function(df, epoch_id):
    df.write.jdbc(url='jdbc:mysql://192.168.245.160:3306/ip2location',  table="transaction_geoip",  properties=db_target_properties, mode="append")
    pass
